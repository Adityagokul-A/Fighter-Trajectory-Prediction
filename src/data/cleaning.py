...
def fuse_altitudes(df):
    """
    Prioritizes geo_altitude, but falls back to barometric_altitude 
    wherever geo_altitude is 0 or NaN.
    """
    print("\n--- Fusing Altitudes ---")
    
    # 1. Treat 0 as missing data by converting it to NaN
    # (If your data natively has NaNs, this handles those too)
    geo_clean = df['Geo Altitude'].replace(0, pd.NA)
    
    # 2. Fill the missing geo data with barometric data
    df['Final_Altitude'] = geo_clean.fillna(df['Barometric Altitude'])
    
    # Quick diagnostic
    geo_count = geo_clean.notna().sum()
    baro_count = len(df) - geo_count
    print(f"Geo Altitude used: {geo_count} times")
    print(f"Baro Altitude used: {baro_count} times")
    print("------------------------\n")
...
if __name__ == '__main__':
    ...
    df_resampled.to_parquet(OUTPUT_PATH, engine='pyarrow', index=False)
    print("Done!")